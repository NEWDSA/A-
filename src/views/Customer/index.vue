<template>
  <div id="cus">
    <div class="fixed">
      <img
        src="@/assets/icon/new-customers_icon.png"
        @click="AddCustomer()"
        v-permission="['Customer.Inquiry.Add-All']"
      />
    ...
    <van-pull-refresh v-model="pullLoading" @refresh="onRefresh" success-text="刷新成功">
        ...
        <nav v-for="(item, index) in CustomList" :key="index">
          <div class="fx">
            <div class="avater">
              ...
              <button v-show="item.InquiryStatus != null">{{ item.InquiryStatus }}</button>
            </div>
          ...
          <div class="fx">
            <aside>
              <div
                class="lc_username"
                @click="Customer_Details(item.InquiryKeyId)"
                style="font-weight: 700; font-size: 16px;"
              >{{ item.CustomerName }}</div>
              <van-button
                v-show="item.InquiryTradeType != null"
                type="primary"
                class="btn_f"
                size="mini"
              >{{ item.InquiryTradeType }}</van-button>
              <van-button
                type="primary"
                class="btn_s"
                size="mini"
                v-if="item.HouseType"
              >{{ item.HouseType }}</van-button>
              <!-- 設置VIP -->
              <van-button
                type="primary"
                class="btn_s"
              ...
              >{{ item.IsVip ? '取消VIP' : '設置VIP' }}</van-button>
            </aside>
            <aside style="font-size: 14px">
              <div>{{ item.InquiryTradeType }}價:</div>
              <template v-if="item.InquiryTradeType == '租购'">{{ item.RentPrice }}元</template>
              <template v-else>
    ...
    >
      ...
      <div class="lc_layout">
        ...
        <div
          class="lc_title"
          v-if="TransType.SysParameter"
        >{{ TransType.SysParameter.ParameterName }}</div>
        ...
        <!-- end 購價 -->
        <!-- 房型 -->
        <div class="lc_title" v-if="RoomType.SysParameter">{{ RoomType.SysParameter.ParameterName }}</div>
        ...
        <div class="lc_title" v-if="Floor.SysParameter">{{ Floor.SysParameter.ParameterName }}</div>
        <div style="display: flex; justify-content: space-evenly; flex-wrap: wrap">
          <template v-for="(item, index) in Floor.Items">
            <div style="display: flex; justify-content: center" @click="e_Floor(index)">
              <div :class="FloorIndex === index ? 'lc_type_active' : 'lc_type'">{{ item.ItemName }}</div>
            </div>
          </template>
        </div>
      </div>
      <div class="lc_footer_btn">
        ...
        <div style="width: 50%; display: flex; justify-content: center" @click="Custom_filter">
          <van-button style="display: inline-block; justify-content: center; flex: 1">確定</van-button>
        </div>
      </div>
    </van-popup>
    <!-- end 内容筛选弹出层 -->
    ...
    >
      ...
      <div class="lc_van_scroll">
        <van-radio-group style="width: 95%" v-model="SearchPerson" @change="KeyReceiver_change">
          <van-cell-group>
            <van-cell
