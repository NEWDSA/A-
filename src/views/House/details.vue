  ...
  <div class="container">
      ...
      <van-sticky>
        <div class="navs">
          ...
          <img
            ...
            alt
          />
          <img v-else @click="collect_i" src="@/assets/icon/nav_collect_transparent_icon.png" alt />
          <!-- </div> -->
        </div>
        <!-- end 顶部导航 -->
      </van-sticky>
    ...
    <div class="lc_layout">
      ...
      <van-swipe class="my-swipe" :autoplay="3000" indicator-color="white" @change="onChange">
        <template v-if="House_detail.Photos != undefined && House_detail.Photos.length > 0">
          <van-swipe-item v-for="(item, index) in House_detail.Photos" :key="index">
            <van-image
              fit="cover"
              @click.stop="preview_show = true; preview_index = index;"
              ref="img"
        ...
        <template v-else>
          <van-swipe-item>
            <van-image ref="img" :src="require('@/assets/img/noImg@2x.png')" />
          </van-swipe-item>
        </template>
      ...
      <div class="property_info">
        <div class="content">
          ...
          <div class="house_tag">
            <!-- <div> -->
            <van-button v-if="House_detail.HouseType" type="default" size="small" class="tag_btn3">業</van-button>
            <van-button
          ...
          <div class="price">
            ...
            <div v-if="House_detail.SaleUnitPrice">{{ House_detail.SaleUnitPrice }}</div>
            <div v-if="House_detail.RentPrice">{{ House_detail.RentPrice }}</div>
            <div v-if="House_detail.RentUnitPrice">{{ House_detail.RentUnitPrice }}</div>
        ...
        <div class="end_content">
          <!-- 業主信息 -->
          <van-collapse v-model="activeName" :close-on-click-mask="false" @change="getOwner">
            <van-collapse-item title="業主信息">
              <div style="text-align:center">
                <van-loading
                  color="#1989fa"
                  v-if="House_Owner_Loading && House_Owner.length < 1"
                >加载中...</van-loading>
              </div>
              <div class="collapse" v-for="(item, index) in House_Owner" :key="index">
                <div>
                  <span class="first">類型:</span>
                  <span>{{ item.TrustorType }}</span>
                </div>
                <div v-if="item.Mobile">
                  <span class="first">手機:</span>
                  <span>{{ item.Mobile }}</span>
                </div>
                <div v-if="item.Tel">
                  <span class="first">座機:</span>
                  <span>{{ item.Tel }}</span>
                </div>
          ...
          <!-- 鑰匙信息 -->
          <van-collapse v-model="activeKey" :close-on-click-mask="false" @change="getKeys">
            <van-collapse-item title="鑰匙信息">
              <template v-if="KeyList.length > 0">
                <div style="text-align:center">
                  <van-loading color="#1989fa" v-if="KeyList_Loading">加载中...</van-loading>
                </div>
                <div class="collapse" v-for="(item, index) in KeyList" :key="index">
                  <div v-if="item.Receiver">
                    <span class="first">收匙人:</span>
                  ...
                  <div v-if="item.PropertyKeyNo">
                    <span class="first">鑰匙編號:</span>
                    <span>{{ item.PropertyKeyNo }}</span>
                  </div>
                  <div v-if="item.KeyCount">
                    <span class="first">鑰匙數量:</span>
                    <span>{{ item.KeyCount }}</span>
                  </div>
                  <div v-if="item.PropKeyStatus">
                    <span class="first">鑰匙箱狀態:</span>
                    <span>{{ item.PropKeyStatus }}</span>
                  </div>
                  <div v-if="item.StatusDescription">
                    <span class="first">狀態描述:</span>
      ...
      <div class="property_info_other">
        ...
        <div class="nav_follow">
          <span>房源跟进</span>
          <van-icon name="arrow" @click="full_Follow" />
        </div>
        <template v-if="FollowUp.length > 0">
    ...
    <div class="lc_contact">
      <div class="tel">
        ...
        <div>
          ...
          <p style="color: #666666">{{ House_detail.PropertyChiefDepartmentName }}</p>
        </div>
        ...
        >新增跟進</van-button>
        <van-button type="info" size="small" color="#54bd46" @click="call">撥打電話</van-button>
      </div>
    </div>
    <van-dialog id="lc_dialog" v-model="show" :show-confirm-button="false" closeOnClickOverlay>
      <template #title>
      ...
      <div class="lc_paper">
        ...
        <div class="container">
          <van-cell-group>
            ...
            <van-field v-model="PaperNo" label="放盤紙編號" placeholder="請輸入放盤紙編號"></van-field>
            <template v-if="PaperStatus == '1'">
              <van-field v-model="Saeles_Start" label="售價範圍開始" placeholder="請輸入售價"></van-field>
              <van-field v-model="Saeles_End" label="售價範圍結束" placeholder="請輸入售價"></van-field>
            </template>
            <template v-if="PaperStatus == '2'">
              <van-field v-model="Rent_Start" label="租金範圍開始" placeholder="請輸入租金"></van-field>
              <van-field v-model="Rent_End" label="租金範圍結束" placeholder="請輸入租金"></van-field>
            </template>
            ...
            <!-- <div class="lc_other_paper">
              <div
                ...
                "
            >-->
            <van-uploader v-model="lc_paper" :after-read="afterRead">
              <template #default>
                <div class="lc_box">
            ...
            />
            <!-- 簽署人 -->
            <van-field v-model="signature" label="簽署人" placeholder="請輸入簽署人"></van-field>
            <!-- 添加聯繫人 -->
            <van-button @click="add_contact" icon="contact" round type="primary" block>添加聯繫人</van-button>
            <template v-for="(item, index) in ContactList">
              <div class="lc_add_contact" @click="remove_contact(item)">{{ item }}</div>
            </template>
            <div style="height: 30px"></div>
            <van-button style="width: 100%" round type="primary" @click="AddPaper">添加</van-button>
          </van-cell-group>
        </div>
      ...
      <van-tabs v-model="ZYVol.active">
        <van-tab title="售成交" :disabled="House_detail.TrustType != 1">
          <van-form @submit="submitSellFrom" ref="SellFrom">
            ...
            <van-field v-model="ZYVol.Space" v-show="this.ZYVol.truckSpace" label="車位" />
            <van-field
              readonly
              clickable
            ...
            <van-field label="座機" v-model="ZYVol.telephone" label-width="4.2em" />
            <van-field label="微信" v-model="ZYVol.weiXin" label-width="4.2em" />
        ...
        <van-tab title="租成交" :disabled="House_detail.TrustType != 2">
          <van-form @submit="submitRentFrom" ref="rentFrom">
            ...
            <div style="margin: 16px">
              <van-button round block type="info" native-type="submit" @click="submitRentFrom">提交</van-button>
            </div>
          </van-form>
        </van-tab>
      </van-tabs>
      ...
      <div class="lc_paper">
        ...
        <div class="container">
          <van-cell-group>
            ...
            <!-- 匙收編號 -->
            <van-field v-model="KeyRecepitNo" type="text" label="匙收據編號" placeholder="請輸入匙收編號" />
            <!-- 鑰匙位置 -->
            <van-field
              v-model="KeyLocation"
              type="text"
            ...
            <van-cell>
              <van-radio-group v-model="KeyBoxLocation" direction="horizontal">
                ...
                <van-radio name="1" @click="KeyBoxLocation = '1'">中原</van-radio>
                <van-radio name="2" @click="KeyBoxLocation = '2'">行家</van-radio>
              </van-radio-group>
            </van-cell>
            <!--聯繫電話-->
            <van-field v-model="Phone" type="number" label="聯繫電話" placeholder="請輸入聯繫電話" />
            <van-field
              v-model="remark"
              rows="2"
              autosize
          ...
          >
            <div style="position: relative; height: 50px">
              <van-button type="primary" @click="Add_Key" block>確認新增</van-button>
            </div>
          </div>
        </div>
      </div>
    ...
    <van-popup
      ...
      v-model="KeyBoxShow"
    >
      <van-picker show-toolbar :columns="KeyBoxList" @confirm="Key_Box_Select" />
    </van-popup>
    <!-- 收匙時間彈窗 -->
    <van-popup
      position="bottom"
    ...
    >
      <van-picker show-toolbar :columns="KeyBoxKeyCountList" @confirm="Key_Box_Key_Count_Select" />
    </van-popup>
    <!-- 收匙人 -->
    <van-popup
      position="bottom"
    ...
    >
      ...
      <!-- 多選 -->
      <template v-show="contact_Type == true">
        <div class="lc_van_scroll">
          <van-checkbox-group v-model="SearchPersonList" @change="KeyReceiver_change">
            <van-cell-group>
              <van-cell
      ...
      <!-- 單選 -->
      <!-- <template v-show="lc_recevier==true">
    ...
    <!-- 圖片預覽組件 -->
    <van-image-preview
      @change="PreviewImagesChange"
      v-model="preview_show"
      :start-position="preview_index"
      :images="images"
    ></van-image-preview>
  </div>
</template>
